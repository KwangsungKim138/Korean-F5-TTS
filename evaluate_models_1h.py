...
# # Step Ranges
# EVAL_STEPS = range(40, 151, 10)  # 60, 70, 80, 90, 100, ..., 150
# UTMOS_STEPS = {
...
# 각 모델별 베스트 체크포인트 스텝 (직접 알맞은 스텝으로 수정해주세요)
BEST_MODELS = {
    "salt_n": 250,
    "grapheme": 250,
    # "grapheme": 50,
...
def build_reference_mapping(test_path):
    ...
    # 1. test.txt에서 순서대로 모든 문장(50개)을 읽어옵니다.
    ...
    # (예: 0번 인덱스는 49번 인덱스와, 1번 인덱스는 48번 인덱스와 매칭)
...
def run_evaluation():
    ...
    for mode, best_step in BEST_MODELS.items():
        ...
        for step in [best_step]:  # 기존 코드 구조 유지를 위해 리스트로 감싸기
