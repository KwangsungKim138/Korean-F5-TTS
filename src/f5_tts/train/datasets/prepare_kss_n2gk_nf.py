"""
Prepare dataset with pipeline: N2gk+ -> g2pk -> 자모 분리 -> Allophone 분류 (N + F).
Mode: N + F (apply_init=False, apply_coda=True with filter=PHONEMES_C_SONORANT, apply_pal=False).
"""
import argparse
import json
import os
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="N2gk+ -> g2pk -> allophone (N + F)."
    )
    parser.add_argument("--name", type=str, default="KSS_n2gk_nf", help="Output dataset name.")
    ...
    # Configuration: N + F
    apply_init = False  # NO Initial voicing tag
    apply_pal = False
    apply_coda = True
    # N (Nasal: ㄴ,ㅁ,ㅇ) + F (Lateral: ㄹ) = Sonorant Codas
