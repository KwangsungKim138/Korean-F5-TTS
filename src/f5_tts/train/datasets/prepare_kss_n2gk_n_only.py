"""
Prepare dataset with pipeline: N2gk+ -> g2pk -> 자모 분리 -> Allophone 분류 (n-only).
Mode: n-only (apply_init=False, apply_coda=True with filter=PHONEMES_N, apply_pal=False).
"""
import argparse
import json
import os
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="N2gk+ -> g2pk -> allophone (n-only)."
    )
    parser.add_argument("--name", type=str, default="KSS_n2gk_n_only", help="Output dataset name.")
    ...
    # Configuration: n-only
    apply_init = False
    apply_pal = False
    apply_coda = True
    coda_filter = PHONEMES_N  # Only nasal consonants get coda tag
