"""
Prepare dataset with pipeline: N2gk+ -> g2pk -> 자모 분리 -> Allophone 분류 (efficient-allophone).
Mode: efficient-allophone (apply_init=True with filter=PHONEMES_I_NO_H, apply_coda=True with filter=PHONEMES_C_SONORANT, apply_pal=False).
"""
import argparse
import json
import os
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="N2gk+ -> g2pk -> allophone (efficient-allophone)."
    )
    parser.add_argument("--name", type=str, default="KSS_n2gk_efficient_allophone", help="Output dataset name.")
    ...
    # Configuration: efficient-allophone
    apply_init = True
    apply_pal = False
    apply_coda = True
    coda_filter = PHONEMES_C_SONORANT
    initial_filter = PHONEMES_I_NO_H
        
    save_dir = data_root / args.name
    save_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"Mode: efficient-allophone")
    print(f"Configuration: Init={apply_init} (filter={initial_filter}), Pal={apply_pal}, Coda={apply_coda} (filter={coda_filter})")
    ...
    if apply_init:
        target_inits = initial_filter if initial_filter else PHONEMES_I_NO_H
        vocab_set |= {c + MARK_INIT for c in target_inits}
    if apply_pal:
        vocab_set |= {p + MARK_PAL for p in PHONEMES_P}
    if apply_coda:
        target_codas = coda_filter if coda_filter else PHONEMES_C_SONORANT
