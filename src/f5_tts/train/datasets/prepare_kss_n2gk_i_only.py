"""
Prepare dataset with pipeline: N2gk+ -> g2pk -> 자모 분리 -> Allophone 분류 (i-only).
Mode: i-only (apply_init=True, apply_coda=False, apply_pal=False).
"""
import argparse
import json
import os
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="N2gk+ -> g2pk -> allophone (i-only)."
    )
    parser.add_argument("--name", type=str, default="KSS_n2gk_i_only", help="Output dataset name.")
