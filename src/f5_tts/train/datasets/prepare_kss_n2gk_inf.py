"""
Prepare dataset with pipeline: N2gk+ -> g2pk -> 자모 분리 -> Allophone 분류 (I+N+F).
Mode: I+N+F (apply_init=True, apply_coda=True with filter=PHONEMES_C_SONORANT, apply_pal=False).
"""
import argparse
import json
import os
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="N2gk+ -> g2pk -> allophone (I+N+F)."
    )
    parser.add_argument("--name", type=str, default="KSS_n2gk_inf", help="Output dataset name.")
    ...
    apply_pal = False
    apply_coda = True
    initial_filter = PHONEMES_I # Default I
    coda_filter = PHONEMES_C_SONORANT  # N (Nasal) + F (Flap/Lateral in coda) = Sonorant Codas
    ...
    if apply_init:
        # Only add init marks for filtered phonemes
        target_inits = initial_filter if initial_filter else PHONEMES_I
