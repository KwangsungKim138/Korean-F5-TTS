"""
Prepare dataset with pipeline: N2gk+ -> g2pk -> 자모 분리 -> 초성 이응 제거.
Mode: no_ieung_g2p (No Allophones).
"""
import argparse
import json
import os
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="N2gk+ -> g2pk -> no_ieung (g2p)."
    )
    parser.add_argument("--name", type=str, default="KSS_no_ieung_g2p", help="Output dataset name.")
    ...
    print(f"Mode: no_ieung_g2p (G2P Applied, No Allophones, Remove Initial Ieung)")
    ...
    for audio_path, text_raw, duration in tqdm(it, desc="Processing"):
        ...
        try:
            ...
            phoneme_list = convert_char_to_no_ieung_g2p([normalized])[0]
    ...
    # Base vocab: Consonants + Vowels + Punctuation
    # Note: 'ㅇ' is in PHONEME_CONSONANTS. It might appear as coda (jongseong).
    # Since we only removed INITIAL ieung, FINAL ieung is still possible.
    phoneme_base = set(PHONEME_CONSONANTS) | set(PHONEME_VOWELS) | punctuation_set
    phoneme_base.discard("")
    
    # Construct vocab
    vocab_set = phoneme_base
