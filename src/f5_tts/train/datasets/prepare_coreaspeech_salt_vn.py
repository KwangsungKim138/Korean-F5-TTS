"""
Prepare CoreaSpeech dataset for salt-vn mode.
Reads metadata_train.txt where the 4th column (index 3) is the pronunciation text.
Applies voiceless initial rule (apply_init=True) and nasal coda rule (apply_coda=True, coda_filter=PHONEMES_N).
...
def convert_pronunciation_to_salt_vn(text: str, use_skip_tc: bool):
    result = []
    eojeols = text.split(' ')
...
def process_line(line, dataset_dir, use_skip_tc):
    ...
    try:
        phoneme_list = convert_pronunciation_to_salt_vn(text_raw, use_skip_tc)
    except Exception:
        return None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="CoreaSpeech -> salt-vn. Output: raw.arrow, duration.json, vocab.txt."
    )
    parser.add_argument("--name", type=str, default="CoreaSpeech_salt_vn", help="Output dataset name.")
    ...
    print(f"Pipeline: CoreaSpeech Pronunciation Text -> salt-vn (skipTC={use_skip_tc})")
