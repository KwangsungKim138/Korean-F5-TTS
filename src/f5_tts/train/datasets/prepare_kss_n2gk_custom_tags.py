"""
Prepare dataset with pipeline: N2gk+ -> g2pk -> 자모 분리 -> Allophone 분류 (Custom Tags).
Supports transcript (KSS pipe-separated) or Parquet input.
Output: raw.arrow, duration.json, vocab.txt; optional raw.parquet.
Modes:
 - i-only: apply_init=True, apply_coda=False, apply_pal=False
 - c-only: apply_init=False, apply_coda=True, apply_pal=False
 - ic:     apply_init=True, apply_coda=True, apply_pal=False
"""
import argparse
import json
import os
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="N2gk+ -> g2pk -> allophone (Custom Tags)."
    )
    parser.add_argument("--tag-mode", type=str, required=True, choices=["i-only", "c-only", "ic"], 
                        help="Select tagging mode: i-only, c-only, or ic. All exclude 'j' tag.")
    parser.add_argument("--name", type=str, default=None, help="Output dataset name. Defaults to KSS_n2gk_{tag_mode}.")
    ...
    # Determine config based on tag-mode
    apply_init = False
    apply_pal = False # Always False per request
    apply_coda = False
    
    if args.tag_mode == "i-only":
        apply_init = True
    elif args.tag_mode == "c-only":
        apply_coda = True
    elif args.tag_mode == "ic":
        apply_init = True
        apply_coda = True
        
    dataset_name = args.name or f"KSS_n2gk_{args.tag_mode.replace('-', '_')}"
    save_dir = data_root / dataset_name
    save_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"Mode: {args.tag_mode}")
    print(f"Configuration: Init={apply_init}, Pal={apply_pal}, Coda={apply_coda}")
    ...
    if apply_coda:
        vocab_set |= {c + MARK_CODA for c in PHONEMES_C}
