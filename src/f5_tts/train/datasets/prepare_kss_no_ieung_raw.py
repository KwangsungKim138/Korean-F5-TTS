"""
Prepare dataset with pipeline: N2gk+ -> 자모 분리 -> 초성 이응 제거.
Mode: no_ieung_raw (No G2P, No Allophones).
"""
import argparse
import json
import os
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="N2gk+ -> raw -> no_ieung (raw)."
    )
    parser.add_argument("--name", type=str, default="KSS_no_ieung_raw", help="Output dataset name.")
    ...
    print(f"Mode: no_ieung_raw (No G2P, No Allophones, Remove Initial Ieung)")
    ...
    for audio_path, text_raw, duration in tqdm(it, desc="Processing"):
        ...
        try:
            ...
            phoneme_list = convert_char_to_no_ieung_raw([normalized])[0]
