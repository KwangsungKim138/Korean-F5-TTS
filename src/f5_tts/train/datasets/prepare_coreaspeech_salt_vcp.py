"""
Prepare CoreaSpeech dataset for salt-vcp mode.
Reads metadata_train.txt where the 4th column (index 3) is the pronunciation text.
Applies i, c, j rules (apply_init=True, apply_pal=True, apply_coda=True) during decomposition.
...
def convert_pronunciation_to_salt_vcp(text: str, use_skip_tc: bool):
    result = []
    eojeols = text.split(' ')
    for eojeol in eojeols:
        for j, syllable in enumerate(eojeol):
            ...
            allophones = _classify_into_allophones(
                ...
                apply_pal=True,
                apply_coda=True,
                coda_filter=None, # applies to all PHONEMES_C
            )
...
def process_line(line, dataset_dir, use_skip_tc):
    ...
    try:
        phoneme_list = convert_pronunciation_to_salt_vcp(text_raw, use_skip_tc)
    except Exception:
        return None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="CoreaSpeech -> salt-vcp. Output: raw.arrow, duration.json, vocab.txt."
    )
    parser.add_argument("--name", type=str, default="CoreaSpeech_salt_vcp", help="Output dataset name.")
    ...
    print(f"Pipeline: CoreaSpeech Pronunciation Text -> salt-vcp (skipTC={use_skip_tc})")
    ...
    for c in PHONEMES_I:
        vocab_set.add(c + MARK_INIT)
    for c in PHONEMES_P:
        vocab_set.add(c + MARK_PAL)
    for c in PHONEMES_C:
        vocab_set.add(c + MARK_CODA)
