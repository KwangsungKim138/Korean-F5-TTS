"""
Prepare dataset with pipeline: N2gk+ -> g2pk -> 자모 분리 -> Allophone 분류.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="N2gk+ -> g2pk -> allophone. Output: raw.arrow, duration.json, vocab.txt. Input: transcript or Parquet."
    )
    parser.add_argument("--name", type=str, default="KSS_n2gk_allophone", help="Output dataset name. Saves to data/{name}.")
    ...
    # Vocab set (same as prepare_kss_allophone)
    punctuation_set = set()
    result = []
    duration_list = []
    ...
    for audio_path, text_raw, duration in tqdm(it, desc="Processing"):
        ...
        try:
            allophone_list = convert_char_to_allophone([normalized])[0]
