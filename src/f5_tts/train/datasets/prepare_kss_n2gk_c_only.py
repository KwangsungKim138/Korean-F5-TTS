"""
Prepare dataset with pipeline: N2gk+ -> g2pk -> 자모 분리 -> Allophone 분류 (c-only).
Mode: c-only (apply_init=False, apply_coda=True, apply_pal=False).
"""
import argparse
import json
import os
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="N2gk+ -> g2pk -> allophone (c-only)."
    )
    parser.add_argument("--name", type=str, default="KSS_n2gk_c_only", help="Output dataset name.")
